=== voltata ===

Contributors: Simon Konstantyner (Sultenhest) http://sultenhest.dk
Tags: black, white, light, dark, responsive layout, one column, two columns, left sidebar, right sidebar, custom background, custom colors, custom header, custom menu, editor style, featured image header, featured images, flexible header, front page posting, full width template, post formats, sticky post, theme options, threaded comments, customizable

Requires at least: 4.0
Tested up to: 4.5.2
Stable tag: 1.2.1
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

A theme called voltata.

== Description ==

Hi. I'm theme called voltata. I’m a simple theme meant for the minimalist blog/website. The very minimalistic looks, opens up lots of opportunities for personal customization either through the customizer or by personal custom CSS.

== Installation ==
	
1. In your admin panel, go to Appearance > Themes and click the Add New button.
2. Click Upload and Choose File, then select the theme's .zip file. Click Install Now.
3. Click Activate to use your new theme right away.

== Features ==

Once you’ve installed the theme, please check out the ‘Customize’ option in the ‘Appearance’ menu

== Versions ==

= 1.0.* - September 21st 2015 =
* Initial release

= 1.1.* - March 23 2016 =
* Minor Fixes
* Added Frontpage template
* Redesign blog list and full width page
* Searchbar option to header navigation
* Menu alignement option
* Fixed embeds

= 1.2.0 - April 27 2016 =
* Now supporting WordPress logo upload in Customizer
* Voltata logo upload now disappreciated, use above instead
* Now functionally supporting post formats: aside, image, video, quote and standard
* Customizer now allows the user to choose which animations to use on page load, aswell as delaytimes
* Greater color customization available in Customizer
* Header slider is now available. Simply check off the check box in the Header Image section in the Customizer
* Shortcode support in text widget
* Supporting 31 fonts now. Also to be found in the Customizer

= 1.2.1 - April 27 2016 =
* Minor fixes

== Credits ==

* CSS based on http://getbootstrap.com/, http://getbootstrap.com/, © 2011-2016 Twitter, Inc, [MIT](http://opensource.org/licenses/MIT)
* Based on Underscores http://underscores.me/, © 2012-2016 Automattic, Inc., [GPLv2 or later](https://www.gnu.org/licenses/gpl-2.0.html)
** This includes navigation.js & skip-link-focus-fix.js
* normalize.css http://necolas.github.io/normalize.css/, © 2012-2016 Nicolas Gallagher and Jonathan Neal, [MIT](http://opensource.org/licenses/MIT)
* animate.css http://daneden.github.io/animate.css/, © 2016 Daniel Eden [MIT](http://opensource.org/licenses/MIT)
* wow.js http://mynameismatthieu.com/WOW/, © 2016 Matt Aussaguel [MIT](http://opensource.org/licenses/MIT)
* modernizr.js http://modernizr.com/, © 2009-2016 Modernizr [MIT](http://opensource.org/licenses/MIT)
* respond.js https://github.com/scottjehl/Respond, © 2011 Scott Jehl [MIT](http://opensource.org/licenses/MIT)
* html5shiv.js https://github.com/afarkas/html5shiv, © 2014 Alexander Farkas [MIT](http://opensource.org/licenses/MIT)
* Fonts: glyphicons http://glyphicons.com, © 2010-2016 Jan Kovarik [MIT](http://opensource.org/licenses/MIT)

* Images used in the screenshot is from the theme author's personal creation and those images are licensed under GNU General Public License v2 or later