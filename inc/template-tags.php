...
function voltata_posted_on() {
	...
	echo '<span class="posted-on muted">' . $posted_on . '</span><span class="byline muted"> ' . $byline . '</span>'; // WPCS: XSS OK.
...
function voltata_posted_on_date_only() {
	...
	echo '<span class="posted-on muted">' . $posted_on . '</span>';
}
endif;
...
function voltata_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		if ( $categories_list && voltata_categorized_blog() ) {
			printf( '<div id="voltata-cat" class="muted"><span class="glyphicon glyphicon-folder-open"></span>
		...
		if ( $tags_list ) {
			printf( '<div id="voltata-tag" class="muted"><span class="glyphicon glyphicon-tags"></span>
			         <span class="tags-links">' . esc_html__( 'Tagged in %1$s', 'voltata' ) . '</span></div>', $tags_list ); // WPCS: XSS OK.
		}
		
		$byline = sprintf(
			esc_html_x( '%s', 'post author', 'voltata' ),
			'<span class="author vcard" class="muted"><a class="url fn n" href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '">' . esc_html( get_the_author() ) . '</a></span>'
		);
	...
	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		echo '<div id="voltata-com" class="muted"><span class="glyphicon glyphicon-comment"></span><span class="comments-link">';
