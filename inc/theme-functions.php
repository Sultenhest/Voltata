 ...
 */
function voltata_header_alignment(){
 ...
 * Getting the menu
 */
function voltata_menu(){
	switch ( get_theme_mod( 'voltata_header_alignment' ) ){
		  case 'center' : $alignment = '2';
		    break;
  		case 'left'   : $alignment = '0';
	  	  break;
	  	case 'right'  : $alignment = '4';
	  	  break;
	  	default       : $alignment = '2';
	  }
    ...
    if( get_theme_mod( 'voltata_display_search' ) != 'hide' ) :
      wp_nav_menu( array( 'theme_location' => 'primary', 'menu_id' => 'primary-menu',
	    ...
	    echo '<div id="menu-search-field" class="col-md-8 col-md-offset-' . $alignment . '">';
			  get_search_form();
	    echo '</div>';
    else :
      wp_nav_menu( array( 'theme_location' => 'primary', 'menu_id' => 'primary-menu',
	  ...
	  echo '<p class="no-menu">No <b>Primary Menu</b> assigned!<br>Head over to <b>Appearance</b> > <b>Menus</b> and assign a <b>Primary Menu</b>.</p>';
  endif;
}
...
function voltata_header_image_slider(){
	...
	$return .= '<div id="carousel-example-generic-header" class="carousel slide">';
  ...
  foreach( $recent_posts as $recent ){
		...
		if($count == 1){
			$return .= '<div id="post-'. $recent['ID'] . '" class="col-md-8' . $classes . '"><a href="' . get_permalink($recent['ID']) . '" rel="bookmark">';
			  ...
			  $return .= '<span class="post-date muted"><a href="' . esc_url( get_permalink( $recent['ID'] ) ) . '" rel="bookmark">' . date('F j, Y', strtotime($recent['post_date'])) . '</a></span>';
			...
			$secondary .= '<div id="post-'. $recent['ID'] . '" class="col-md-12 col-xs-6' . $classes . '"><a href="' . get_permalink($recent['ID']) . '" rel="bookmark">';
			  ...
			  $secondary .= '<span class="post-date muted"><a href="' . esc_url( get_permalink( $recent['ID'] ) ) . '" rel="bookmark">' . date('F j, Y', strtotime($recent['post_date'])) . '</a></span>';
			$secondary .= '</div>';
		}
